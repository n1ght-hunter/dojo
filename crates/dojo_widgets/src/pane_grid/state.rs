...
use super::{Axis, Configuration, Direction, Node, Pane, PaneConstraints, Split};
use crate::pane_grid::{Edge, Region, Target};
use iced::{Point, Size};
...
impl<T> State<T> {
    ...
    /// Sets the constraints for a specific pane.
    pub fn set_constraints(&mut self, pane: Pane, constraints: PaneConstraints) {
        self.constraints.insert(pane, constraints);
    }

    /// Gets the constraints for a specific pane.
    pub fn get_constraints(&self, pane: Pane) -> PaneConstraints {
        self.constraints.get(&pane).copied().unwrap_or_default()
    }

    /// Returns a reference to the constraints map.
    pub fn constraints(&self) -> &BTreeMap<Pane, PaneConstraints> {
        &self.constraints
    }
