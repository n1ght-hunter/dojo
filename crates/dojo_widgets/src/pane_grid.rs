...
/// Constraints for a pane's size.
#[derive(Debug, Clone, Copy, Default)]
pub struct PaneConstraints {
    /// Minimum width in pixels. None means no minimum.
    pub min_width: Option<f32>,
    /// Maximum width in pixels. None means no maximum.
    pub max_width: Option<f32>,
    /// Minimum height in pixels. None means no minimum.
    pub min_height: Option<f32>,
    /// Maximum height in pixels. None means no maximum.
    pub max_height: Option<f32>,
}

impl PaneConstraints {
    /// Creates new constraints with all values set to None (no constraints).
    pub fn new() -> Self {
        Self::default()
    }
    ...
    /// Returns the effective minimum size for the given axis.
    pub fn min_size(&self, axis: Axis) -> f32 {
        match axis {
            Axis::Horizontal => self.min_height.unwrap_or(0.0),
            Axis::Vertical => self.min_width.unwrap_or(0.0),
        }
    }

    /// Returns the effective maximum size for the given axis.
    pub fn max_size(&self, axis: Axis) -> Option<f32> {
        match axis {
            Axis::Horizontal => self.max_height,
            Axis::Vertical => self.max_width,
        }
    }
}
    ...
    Renderer: iced::advanced::Renderer,
{
    ...
    constraints: &'a std::collections::BTreeMap<Pane, PaneConstraints>,
    panes: Vec<Pane>,
    ...
    fn grid_interaction(
        ...
        let resize_axis = action.picked_split().map(|(_, axis)| axis).or_else(|| {
            resize_leeway.and_then(|leeway| {
                ...
                let splits = node.split_regions_with_constraints(
                    self.spacing,
                    self.min_size,
                    bounds.size(),
                    self.constraints,
    ...
    Renderer: iced::advanced::Renderer,
{
    ...
    fn layout(
        ...
        let regions = self.internal.layout().pane_regions_with_constraints(
            self.spacing,
            self.min_size,
            bounds,
            self.constraints,
